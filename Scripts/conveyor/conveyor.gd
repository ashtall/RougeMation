...
func give_resource():
	if get_next_item(output_dir):
		if get_next_item(output_dir)["name"] == "conveyor":
			await get_tree().create_timer(conveyor_speed).timeout
			var conveyor = get_next_item(output_dir)
...
func set_input_output_dir():
	input_dir = direction_facing
	match item.name:
		"conveyor_right":
			output_dir = Autoload.get_direction_facing(
				get_node("Pivot").rotation_degrees + Vector3(0, -90, 0)
			)
		"conveyor_left":
			output_dir = Autoload.get_direction_facing(
				get_node("Pivot").rotation_degrees + Vector3(0, 90, 0)
